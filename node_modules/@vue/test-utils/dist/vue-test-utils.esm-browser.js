...
/**
 * @vue/test-utils v2.0.0-rc.11
 * (c) 2021 Lachlan Miller
 * Released under the MIT License
 */

import * as Vue from 'vue';
import { defineComponent, h, transformVNodeArgs, Transition, TransitionGroup, nextTick, setDevtoolsHook, getCurrentInstance, reactive, createApp } from 'vue';
...
function mergeGlobalProperties(mountGlobal) {
    ...
    return {
        mixins: __spreadArray(__spreadArray([], (configGlobal.mixins || [])), (mountGlobal.mixins || [])),
        plugins: __spreadArray(__spreadArray([], (configGlobal.plugins || [])), (mountGlobal.plugins || [])),
        stubs: stubs,
...
var getOriginalVNodeTypeFromStub = function (type) { return stubsMap.get(type); };
    ...
    transformVNodeArgs(function (args, instance) {
        ...
        if (isComponent$1(type) || isFunctionalComponent(type)) {
            ...
            if (isComponent$1(stub)) {
                var stubFn_1 = isFunctionalComponent(stub) ? stub : null;
                var specializedStub = stubFn_1
                    ? function () {
                        var args = [];
            ...
            if (stub === true || shallow) {
                ...
                if (!name_1) {
                    name_1 = registeredName || componentName;
                }
                if (!isComponent$1(type)) {
                    throw new Error('Attempted to stub a non-component');
                }
                var propsDeclaration = type.props || {};
                var newStub = components[name_1];
                if (!newStub) {
                    newStub = createStub({
                        name: name_1,
                        propsDeclaration: propsDeclaration,
                        renderStubDefaultSlot: renderStubDefaultSlot
                    });
                    components[name_1] = newStub;
...
function processSlot(source, Vue$1) {
    ...
    var Component = {
        inheritAttrs: false,
        render: createRenderFunction(Vue$1),
        components: {
...
function aggregateChildren(nodes, children) {
    if (children && Array.isArray(children)) {
        var reversedNodes = __spreadArray([], children).reverse().filter(nodesAsObject);
...
var VueWrapper = /** @class */ (function (_super) {
    ...
    VueWrapper.prototype.find = function (selector) {
        var result = this.parentElement['__vue_app__']
    ...
    VueWrapper.prototype.findAll = function (selector) {
        var results = this.parentElement['__vue_app__']
    ...
    VueWrapper.prototype.isVisible = function () {
        var domWrapper = new DOMWrapper(this.element);
    ...
    return {
        created: function () {
            ...
            for (var _i = 0, _b = Object.entries(data); _i < _b.length; _i++) {
                ...
                getCurrentInstance().data = reactive(__assign(__assign({}, getCurrentInstance().data), (_a = {}, _a[k] = v, _a)));
            }
        }
    };
};
...
function mount(originalComponent, options) {
    ...
    function slotToFunction(slot) {
        if (typeof slot === 'object') {
            ...
            if ('template' in slot && slot.template) {
                return function () { return h(slot, props); };
            }
            return function () { return slot; };
        }
        ...
        if (typeof slot === 'string') {
            ...
            if (isHTML(slot)) {
                return function (props) { return h(processSlot(slot), props); };
            }
    ...
    var slots = (options === null || options === void 0 ? void 0 : options.slots) &&
        Object.entries(options.slots).reduce(function (acc, _a) {
            ...
            if (Array.isArray(slot)) {
                var normalized_1 = slot.reduce(function (acc, curr) {
                    var slotAsFn = slotToFunction(curr);
                    if (isObject$2(curr) && 'render' in curr) {
                        var rendered = h(slotAsFn);
                        return acc.concat(rendered);
                    }
                    return acc.concat(slotAsFn());
                }, []);
                acc[name] = function () { return normalized_1; };
                return acc;
            }
    ...
    if (options === null || options === void 0 ? void 0 : options.data) {
        ...
        component.mixins = __spreadArray(__spreadArray([], (component.mixins || [])), [
            dataMixin
        ]);
    }
    ...
    if (global.plugins) {
        for (var _c = 0, _d = global.plugins; _c < _d.length; _c++) {
            ...
            if (Array.isArray(plugin)) {
                app.use.apply(app, __spreadArray([plugin[0]], plugin.slice(1)));
                continue;
            }
            app.use(plugin);
        }
    ...
    if (global === null || global === void 0 ? void 0 : global.stubs) {
        for (var _o = 0, _p = Object.entries(global.stubs); _o < _p.length; _o++) {
            ...
            if (stub === true) {
                var stubbed = createStub({
                    name: name_1,
                    renderStubDefaultSlot: global === null || global === void 0 ? void 0 : global.renderStubDefaultSlot
                });
                // default stub.
                app.component(name_1, stubbed);
            }
            else {
                // user has provided a custom implementation.
                app.component(name_1, stub);
            }
        }
    }
    // mount the app!
