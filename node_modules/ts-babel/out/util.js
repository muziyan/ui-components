...
async function readProjectMetadata(packageDir) {
    const packageDirs = bluebird.filter((await fs_1.promises.readdir(packageDir)).filter(it => !it.includes(".")).sort(), it => {
        return fs_1.promises.stat(path.join(packageDir, it, "tsconfig.json"))
            .then(it => it.isFile())
            .catch(() => false);
    });
    return await bluebird.map(packageDirs, it => fs_1.promises.readFile(path.join(packageDir, it, "package.json"), "utf-8").then(it => JSON.parse(it)), { concurrency: 8 });
}
...
async function build(basePath, transpilator) {
    ...
    const jsonResult = ts.parseConfigFileTextToJson(tsConfigPath, await fs_1.promises.readFile(tsConfigPath, "utf8"));
